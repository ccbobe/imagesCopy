...
public class UserDao extends BaseDao {
	 ...
	 */
	public UserDao() {
		select.append("SELECT t.id,t.name,t.password");
		select.append(" FROM user t WHERE 1=1");

		insert.append("INSERT INTO user (name,password) ");
		insert.append(" VALUES (:name,:password)");
	}
	...
	public int save(User vo) {
		...
		sql.append("INSERT INTO user (id,name,password)");
		sql.append(" VALUES (?,?,?)");
		Object[] params = { vo.getId(), vo.getName(), vo.getPassword() };
	...
	public int update(User vo) {
		StringBuilder sql = new StringBuilder();
		sql.append("UPDATE user SET name=?,password=? ");
		sql.append(" WHERE id=? ");
		Object[] params = { vo.getName(), vo.getPassword(), vo.getId() };
		logger.info(SqlUtil.showSql(sql.toString(), params));// 显示SQL语句
		return jdbcTemplate.update(sql.toString(), params);
	}
